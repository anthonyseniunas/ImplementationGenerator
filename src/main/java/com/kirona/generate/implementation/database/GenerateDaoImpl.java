...
//            refCode.setSyncToMobile(rs.getString("SYNC_TO_MOBILE_FLAG").equals("Y")?true:false);
  ...
  public RefCode getRefCode(String domain,String lowValue)
  {
    List<RefCode> refCodesList = jdbcTemplate.query(
        "SELECT "+
        "  RV_LOW_VALUE, " + 
...
//        "  RV_DATA_X_02 " + 
//        "  SYNC_TO_MOBILE_FLAG " + 
        " FROM KIR_REF_CODES WHERE RV_DOMAIN=? AND RV_LOW_VALUE=?",
        new Object[] {domain,lowValue},
        new RowMapper<RefCode>()
        {
          public RefCode mapRow(ResultSet rs, int rownum) throws SQLException
          {
...
//            refCode.setSyncToMobile(rs.getString("SYNC_TO_MOBILE_FLAG").equals("Y")?true:false);
    ...
    if (refCodesList!=null && refCodesList.size()>0)
    {
      return refCodesList.get(0);
    }
    else
    {
      return null;
